            ...
            <form action="{{ route('admin.products.create') }}" method="POST" class="text-lg">
                ...
                <div class="bg-white border border-gray-200 rounded-lg py-7 px-8 space-y-5">
                    ...
                    <div class="space-y-1">
                        ...
                        <p class="text-gray-500 text-sm">Заглавието на продукта ще се използва за показване в списъци и страници.</p>
                        @error('name')
                    ...
                    <div class="grid xl:grid-cols-2 gap-5">
                        ...
                        <div class="space-y-1">
                            ...
                            <p class="text-gray-500 text-sm">Промоционалната цена ще се показва вместо основната, когато е зададена.</p>
                            @error('sale_price')
                                <div class="text-red-500">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <textarea name="short_description" id="short_description" rows="3"
                            placeholder="Въведете кратко описание продукта" class="form-control">{{ old('short_description') }}</textarea>
                    ...
                    <div class="space-y-1">
                        ...
                        <textarea name="description" id="description" rows="10"
                            placeholder="Въведете подробно описание продукта" class="form-control">{{ old('description') }}</textarea>
                        <p class="text-gray-500 text-sm">Подробно описание за страницата на продукта, включително характеристики и детайли.</p>
                        @error('description')
                            <div class="text-red-500">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        <select name="category_id" class="form-control">
                            ...
                            @foreach($categoriesForDropdown as $option)
                                <option value="{{ $option['id'] }}"
                                    @if(old('category_id') == $option['id'] || (isset($product) && $product->categories->contains('id', $option['id'])))
                                        selected
                                    @endif>
                                    {{ $option['name'] }}
                                </option>
